...
class OptInit:
    def __init__(self):
        ...
        # attack args
        parser.add_argument('--attack', type=str, default='random', help='the implemented attack: random, NU_attack, tar_NU_attack, NB_attack, tar_NB_attack')
        parser.add_argument('--left_ratio', type=float, default=0.1, help='the left points ratio for the L0 attack')
        parser.add_argument('--att_epsilon', default=0.01, type=float, help='the ability of the attack')
        ...
        parser.add_argument('--batch_size', default=1, type=int, help='mini-batch size (default:16)')
        ...
        parser.add_argument('--lr_decay_rate', default=0, type=float, help='learning rate decay') # 0.5
        ...
        # model args
        parser.add_argument('--pretrained_model', type=str, help='path to pretrained model(default: none)', default='sem_seg_dense-res-edge-28-64-ckpt_best_model.pth')
        ...
        parser.add_argument('--dropout', default=0, type=float, help='ratio of dropout') # 0.3

        # dilated knn
        parser.add_argument('--epsilon', default=0, type=float, help='stochastic epsilon for gcn')# 0.2 impact the randomization
    ...
    def _set_seed(self, seed=0):
        # print("config:", self.args.dropout)
        random.seed(seed)
        np.random.seed(seed)
