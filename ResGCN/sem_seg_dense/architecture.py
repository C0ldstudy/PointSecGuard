...
class DenseDeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.knn = DenseDilatedKnnGraph(k, 1, stochastic, epsilon) # Find the neighbors' indices based on dilated knn
    ...
    def forward(self, inputs):
        ...
        feats = torch.cat(feats, dim=1) # (1, 1792, 4096, 1)
        fusion = torch.max_pool2d(self.fusion_block(feats), kernel_size=[feats.shape[2], feats.shape[3]]) # ([1, 1024, 1, 1])
        fusion = torch.repeat_interleave(fusion, repeats=feats.shape[2], dim=2) # ([1, 1024, 4096, 1])
        # print(fusion[0,0,:10])

        return self.prediction(torch.cat((fusion, feats), dim=1)).squeeze(-1)#, temp
