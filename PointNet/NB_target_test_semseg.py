...
def parse_args():
    ...
    parser.add_argument('--target', default=7, type=int, help='the traget label of the target points')
...
def main(args):
    ...
    if True:
        ...
        with open(os.path.join(experiment_dir, 'log_'+ str(ori)+ '_' + timestr + '.txt'), 'w') as f:
            ...
            for simple_index in tqdm(range(args.num_votes), total=args.num_votes):
                ...
                for sbatch in range(s_batch_num):
                    ...
                    if count == 0:
                        ...
                        attack = torchattacks.tar_NB_attack(classifier, eps=0.5, alpha=0.1, iters=500, target=target, mask=mask[0])
                    ...
                    if count != 0:
                        ...
                        with open(os.path.join(experiment_dir, 'log_'+ str(ori)+ '_' +timestr+'.txt'),'a+') as f:
